...
class SwitchboardIntegration(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="The unique ID of the switchboard integration.")
    ...
    integration_id: StrictStr = Field(description="Id of the integration that should deliver events routed by the switchboard.", alias="integrationId")
    integration_type: StrictStr = Field(description="Type of integration that should deliver events routed by the switchboard. If referencing an OAuth integration, the clientId issued by Sunshine Conversations during the OAuth partnership process will be the value of integrationType.", alias="integrationType")
    deliver_standby_events: StrictBool = Field(description="Setting to determine if webhooks should be sent when the switchboard integration is not in control of a conversation (standby status)", alias="deliverStandbyEvents")
    ...
    __properties: ClassVar[List[str]] = ["id", "name", "integrationId", "integrationType", "deliverStandbyEvents", "nextSwitchboardIntegrationId", "messageHistoryCount"]

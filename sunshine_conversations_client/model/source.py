...
class Source(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="An identifier for the channel from which a message originated. May include one of api, sdk, messenger, or any number of other channels.")
    ...
    client: Optional[Client] = Field(default=None, description="The client from which the user authored the message or activity, if applicable. This field is not applicable in API responses, it is used only in webhook payloads if the `includeFullSource` option is enabled.")
    device: Optional[Device] = Field(default=None, description="The device from which the user authored the message or activity, if applicable. This field is not applicable in API responses, it is used only in webhook payloads if the `includeFullSource` option is enabled.")
    __properties: ClassVar[List[str]] = ["type", "integrationId", "originalMessageId", "originalMessageTimestamp", "client", "device"]

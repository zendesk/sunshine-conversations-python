...
class ClientAddEventAllOfPayload(BaseModel):
    """
    The payload of the event. The contents of this object depend on the type of event.
    """ # noqa: E501
    conversation: Optional[ConversationTruncated] = Field(default=None, description="The conversation associated with the creation of the client. This field is only present when the reason is `channelLinking` and when attaching the client to a specific conversation. ")
    user: Optional[UserTruncated] = Field(default=None, description="The user associated with the client.")
    client: Optional[Client] = Field(default=None, description="The client that was just created.")
    reason: Optional[StrictStr] = Field(default=None, description="The reason for which the client was added. * `channelLinking` - The client was created as a result of initiating a channel link. * `sdkLogin` - The client was created as a result of logging into an SDK device. * `authCode` - The client was created as a result of initializing an SDK client with an `authCode`. ")
    ...
    __properties: ClassVar[List[str]] = ["conversation", "user", "client", "reason", "source"]
    ...
    def reason_validate_enum(cls, value):
        ...
        if value not in set(['channelLinking', 'sdkLogin', 'authCode']):
            raise ValueError("must be one of enum values ('channelLinking', 'sdkLogin', 'authCode')")
        return value
